...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function syncComponentsImmediate(newState, oldState) {
        try {
            ...
            console.error('main.js: Error during state sync:', error);
        }
    }
    ...
    async function init() {
        try {
            ...
            // Step 2: BatchServiceの初期化（Dashboard参照を追加）
            batchService = new BatchService({
                showToast: showToast,
                urlManager: urlManager,
                dashboard: dashboard,           // Dashboard参照追加
                authService: authService,
                refreshDashboard: refreshDashboard
            });
            
            // 【重要修正】ProgressMonitorにDashboard参照を設定
            if (batchService.progressMonitor) {
                batchService.progressMonitor.setStorageService(storageService);
                batchService.progressMonitor.setDashboard(dashboard);
            }
            ...
            if (executeFromUrlTabButton && stopFromUrlTabButton) {
                ...
                await urlManager.restoreButtonStateFromStorage(
                    enhancedExecuteHandler,
                    handleSendingStop
                );
            }

            // Step 5: リアルタイム状態同期システムを設定
